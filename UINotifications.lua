...
function notify.init(guiname : string)
...
function notify.notify(notification : string, duration : number)
    ...
    task.spawn(function()
        local TweenIn = TweenService:Create(Notification, TweenInfo.new(.15), {Size = UDim2.new(0, bounds.X + 25, 0.042, 0)})
        TweenIn:Play()
        TweenIn.Completed:Wait()
        local TweenBar = TweenService:Create(Bar, TweenInfo.new(duration or 5), {Size = UDim2.new(0, 0, 1, 0)})
        TweenBar:Play()
        TweenBar.Completed:Wait()
        local TweenOut = TweenService:Create(Notification, TweenInfo.new(.15), {Size = UDim2.new(0, 0, 0.042, 0)})
        TweenOut:Play()
